...
exports.requirements = function (html) {
  var cell = $(html)
    .find('table[width="100%"] > tr:contains("先修课程") > td > div[align="left"]')
    .first()
    .text()
    .trim()
  ...
  return cell.split(' 或 ').map(function (or) {
    return or.split(' 和 ').map(function (and) {
      var match = and.trim().match(/.+\(([A-Z0-9]+)\)$/)
      return match[1]
    })
  })
}
